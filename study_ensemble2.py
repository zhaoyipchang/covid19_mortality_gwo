...
def objective_ensemble_reduced1(trial):
    ...
    prob1 = clf1.fit(X_train[selected_attributes1], y_train).predict_proba(X_val[selected_attributes1])[::,1]
    prob2 = clf2.fit(X_train[selected_attributes1], y_train).predict_proba(X_val[selected_attributes1])[::,1]
    prob3 = clf3.fit(X_train[selected_attributes1], y_train).predict_proba(X_val[selected_attributes1])[::,1]
    ensemble_prob = prob1*norm_w1 + prob2*norm_w2 + prob3*norm_w3
    aucroc = roc_auc_score(y_val, ensemble_prob)
    return aucroc

best_position = gwo(objective_ensemble_reduced1, max_iter=1000, n=50, dim=1, minx=0.1, max=0.9)
...
prob1_reduced1 = clf1_reduced1.fit(X_train[selected_attributes1], y_train).predict_proba(X_test[selected_attributes1])[::,1]
prob2_reduced1 = clf2_reduced1.fit(X_train[selected_attributes1], y_train).predict_proba(X_test[selected_attributes1])[::,1]
prob3_reduced1 = clf3_reduced1.fit(X_train[selected_attributes1], y_train).predict_proba(X_test[selected_attributes1])[::,1]
...
attributes_r1 = list(X_train[selected_attributes1].columns)
